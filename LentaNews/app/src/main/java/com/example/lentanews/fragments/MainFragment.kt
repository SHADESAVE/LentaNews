...
class MainFragment : Fragment() {
    ...
    private fun getResult(): ArrayList<RowType>  {
        ...
        val asyncRequest = DownloadNews()
        val news = asyncRequest.execute().get()
        (activity as MainActivity).newsList = news

        return arrayListOf(
                HeaderRowType("Top7"),
                NewsRowType(news.top7[0].title, news.top7[0].description, news.top7[0].imageUrl, news.top7[0].pubDate, news.top7[0].link),
                NewsRowType(news.top7[1].title, news.top7[1].description, news.top7[1].imageUrl, news.top7[1].pubDate, news.top7[1].link),
